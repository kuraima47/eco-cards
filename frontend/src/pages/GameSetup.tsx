...
const GameSetup: React.FC = () => {
  ...
  const [guestUsernames, setGuestUsernames] = useState<Record<number, string>>({});

  useEffect(() => {
    const loadData = async () => {
      try {
        const [decksData, usersData] = await Promise.all([
    ...
    try {
      ...
      if (groupId > 0 && id) {
        ...
        if (groupToDelete?.players.length) {
          await Promise.all(
            groupToDelete.players.map(player => 
              groupPlayerService.deleteGroupPlayer(groupId, player.username)
            )
          );
        }
      ...
      setGroupErrors(prev => ({ ...prev, [groupId]: undefined }));
  ...
  const handleAddPlayersToGroup = (groupId: number, userIds: number[]) => {
    ...
    const validUsers = users.filter(user => 
      user.userId && userIds.includes(user.userId)
    );
  ...
  const handleAddGuestToGroup = (groupId: number) => {
    ...
    if (!username) return;
    sessionService.sendLink("1", username);
    // setGroups(prev => prev.map(group => 
    //   group.groupId === groupId ? {
    //     ...group,
    //     players: [
    //       ...group.players,
    //       { groupId, username, userId: null }
    //     ]
    //   } : group
    // ));
  ...
  const handleSubmit = async () => {
    try {
      ...
      for (const group of updatedGroups) {
        ...
        const playersToAdd = group.players.filter(p => 
          !existingPlayers.some(ep => ep.username === p.username)
        );
        const playersToRemove = existingPlayers.filter(ep => 
          !group.players.some(p => p.username === ep.username)
        );
        ...
        await Promise.all([
          ...
          ...playersToRemove.map(p => 
            groupPlayerService.deleteGroupPlayer(group.groupId, p.username)
          )
        ]);
      }
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        onChange={e => setGuestUsernames(prev => ({
                          ...prev,
